...
t.update_by([
    ...
    rolling_count_where_tick(col="UpTicks", filters="Price > PrevPrice", rev_ticks=10),
...
t.update_by([
    ...
    rolling_avg_time("Timestamp", cols=["MA_Centered = Price"],
                     rev_time="PT15s", fwd_time="PT15s"),

    # Count conditions in window
    rolling_count_where_time("Timestamp", col="UpMoves",
                            filters="Price > PrevPrice", rev_time="PT10m"),
], by=["Sym"])
```
...
macd = t.update_by([
    ...
    ema_tick(decay_ticks=9, cols=["Signal = MACD"])
], by=["Sym"]).update([
...
bands = t.update_by([
    ...
    "UpperBand = MA + 2 * StdDev",
    "LowerBand = MA - 2 * StdDev"
])
```
...
twap = t.update_by([
    rolling_avg_time("Timestamp", cols=["TWAP = Price"], rev_time="PT1h")
], by=["Sym"])
```
